...
| `/chat <@username หรือ ID> [n]` | ดูประวัติการสนทนาย้อนหลังระหว่าง User กับ Bot (เช่น `/chat 5125375696 20`) |
| `/reply <@username หรือ ID> <msg>` | ส่งข้อความตอบกลับผู้ใช้ทาง DM ในนามทีมงานแอดมิน (เช่น `/reply 5125375696 สวัสดีครับ`) |
| `/sync` หรือ `/sync_channel` | สั่งซิงค์และกู้คืนผู้ใช้ที่ค้าง PENDING ทั้งหมด (ตรวจพบในห้องจะเปิดใช้งาน ACTIVE ทันที / เตะคนที่หมดเวลาแล้ว) |
...
| `/reset_user <@username หรือ ID>` | รีเซ็ต/ลบประวัติและบัญชีผู้ใช้ทั้งหมด (Factory Reset) พร้อมเตะออกจากห้อง เพื่อทดสอบระบบตั้งแต่เริ่มใหม่ |
| `/reset_trial <@username หรือ ID>` | คืนสิทธิ์ทดลองใช้ฟรี 15 นาทีให้ผู้ใช้คนนั้นใหม่ โดยไม่ลบประวัติเดิม |
